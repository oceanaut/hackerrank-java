...
public class TripleSum1 {
	...
	static long binarysearchCustom(int[] arr, int key) {
		int index=-1,low=0,mid=0, high=arr.length-1;
		while (high >= low) {
			mid= low + (high-low) /2;
			if ( key >= arr[mid] ) {
				index=mid;
    ...
    static long triplets(int[] a, int[] b, int[] c) {
        ...
        long count = 0;
        for (int second : bSet) {
            long ac = Math.abs(binarysearchCustom(aSet, second) + 1);
            long cc = Math.abs(binarysearchCustom(cSet, second) + 1);
            count += ac * cc;
        }
        return count;
    }
